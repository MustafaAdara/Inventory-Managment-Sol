...
namespace MOFA.StockManagement.Domain.Patterns.Repositories
{
    public class Query<TEntity> : IQuery<TEntity> where TEntity : class
    {
        ...
        public virtual IQuery<TEntity> OrderBy(Expression<Func<TEntity, object>> keySelector)
        {
            ...
            {
                _orderedQuery = _query.OrderBy(keySelector);
            }
            else
            {
                _orderedQuery.OrderBy(keySelector);
            }

            return this;
        }

        public virtual IQuery<TEntity> OrderBy(string propertyName)
        {
            if (_orderedQuery == null)
            {
                _orderedQuery = _query.OrderBy(propertyName);
            }
            else
            {
                _orderedQuery.OrderBy(propertyName);
            }
        ...
        public virtual IQuery<TEntity> OrderByDescending(Expression<Func<TEntity, object>> keySelector)
        {
            if (_orderedQuery == null)
            {
                _orderedQuery = _query.OrderByDescending(keySelector);
            }
            else
            {
                _orderedQuery.OrderByDescending(keySelector);
            }

            return this;
        }
        public virtual IQuery<TEntity> OrderByDescending(string propertyName)
        {
            if (_orderedQuery == null)
            {
                _orderedQuery = _query.OrderByDescending(propertyName);
            }
            else
            {
                _orderedQuery.OrderByDescending(propertyName);
            }
        ...
        public virtual async Task<IEnumerable<TEntity>> SelectSqlAsync(string sql, object[] parameters,
            CancellationToken cancellationToken = default)
        {
            return await ((DbSet<TEntity>)_query).FromSqlRaw(sql, parameters).ToListAsync(cancellationToken);
        }
